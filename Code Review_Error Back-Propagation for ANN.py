...
# 建议：偏置向量形状建议为 (hidden_dim,) 和 (output_dim,)，利用广播机制，避免显式 (1, N) 带来的维度错位风险
...
def sigmoid(x):                          #Optimize the activation function sigmoid, to make the results less overflow
    ...
    return y
# 建议：此分段实现虽可缓解溢出，但可能牺牲部分性能；也可考虑直接使用 scipy.special.expit 或 1/(1+np.exp(-np.clip(x, -500, 500)))
...
def backward_output_neuron(cache, T):   
    ...
    deerror = output - T                                  #(1,4), partial derivative of total error
        # 建议：明确损失函数定义。若为 1/2 * sum((y-t)^2)，则 deerror 应为 output - T；但此处缺少 1/N 平均，按单样本训练可接受
    ...
    return dw, db
# 建议：此函数内部重复计算了前向传播，与主循环中的前向计算重复，既浪费又容易不一致；应直接传入 cache 和已计算的 actived_Y 或必要的中间量
...
def backward_input_neuron(cache, cathe_pre, T):   
    ...
    # 建议：step 计算包含了 output_pre - T 和激活导数，实际应为上层传入的 delta；这里重复计算且耦合了具体损失
...
# 建议：硬编码迭代次数，建议改为变量 n_epochs，或增加早停条件
...
# 建议：梯度累加器建议与权重偏置形状一致，dB 形状最好是 (hidden_dim,) 和 (output_dim,)，避免维度不匹配
    ...
    for n in [0, 1, 2, 3]:                                                    #four groups of training patterns               
        ...
        dB1 = dB1 + part_dB1
        E = E + En
# 建议：计算完所有样本后应该除以样本数得到平均梯度，否则更新步长受样本数量影响；同时误差累加可用 +=
